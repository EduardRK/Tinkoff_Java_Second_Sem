...
public class JdbcLinkRepository implements LinkRepository {
    ...
    public List<Link> getAllLinksUpdateLastCheck() {
        List<Link> linkList = jdbcClient
            .sql("SELECT * FROM link")
            .query(Link.class)
            .list();

        jdbcClient
            .sql("UPDATE link SET last_check = ?")
    ...
    public List<Link> getAllLinksUpdateLastCheckWithFilter(Duration earlyThen) {
        ...
        List<Link> linkList = jdbcClient
            .sql("SELECT * FROM link WHERE last_check < ?")
            .param(minuses)
            .query(Link.class)
            .list();

        jdbcClient
            .sql("UPDATE link SET last_check = ? WHERE last_check < ?")
    ...
    public long addLink(Link link) {
        Long linkExist = jdbcClient
            .sql("SELECT COUNT(*) FROM link WHERE uri = ?")
            .param(link.uri())
            .query(Long.class)
            .single();
        ...
        if (linkExist == 0) {
            return jdbcClient
                .sql("INSERT INTO link(uri, last_check, last_update) VALUES (?, ?, ?) RETURNING id")
                .params(link.uri(), link.lastCheck(), link.lastUpdate())
                .query(Long.class)
                .single();
        } else {
            return jdbcClient
                .sql("SELECT id FROM link WHERE uri = ?")
                .param(link.uri())
                .query(Long.class)
                .single();
        }
    ...
    public long removeLink(Link link) {
        return jdbcClient
            .sql("DELETE FROM link WHERE uri = ? RETURNING id")
            .param(link.uri())
            .query(Long.class)
            .single();
    }
    ...
    public void updateLastUpdateTime(Link link, OffsetDateTime now) {
        jdbcClient
            .sql("UPDATE link SET last_update = ? WHERE uri = ?")
            .params(now, link.uri())
            .update();
    }
    ...
    public void updateAllLastUpdateTime(OffsetDateTime now) {
        jdbcClient
            .sql("UPDATE link SET last_update = ?")
            .param(now)
            .update();
    }
}
